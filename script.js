...
const state = {
  ...
  products: [
    { id: "p1", sku: "ELQ-MNG-30", name: "Mango Ice 30ml", categoryId: "c1", brandId: "b4", cost: 6.5, price: 14.99, stock: 42, unit: "bottle" },
    { id: "p2", sku: "ELQ-BLR-30", name: "Blue Razz 30ml", categoryId: "c1", brandId: "b4", cost: 6.5, price: 14.99, stock: 8, unit: "bottle" },
    { id: "p3", sku: "DIS-GEEK-5K", name: "Geekstick 5000 Puff", categoryId: "c2", brandId: "b6", cost: 9.2, price: 19.99, stock: 3, unit: "unit" },
    { id: "p4", sku: "DIS-FROST-6K", name: "Frost Bar 6000 Puff", categoryId: "c2", brandId: "b3", cost: 10.0, price: 21.99, stock: 27, unit: "unit" },
    { id: "p5", sku: "MOD-VLT-80W", name: "Volt X80 Box Mod", categoryId: "c3", brandId: "b2", cost: 24.0, price: 49.99, stock: 14, unit: "unit" },
    { id: "p6", sku: "KIT-CLD9-ST", name: "CloudNine Starter Kit", categoryId: "c3", brandId: "b1", cost: 18.5, price: 39.99, stock: 6, unit: "unit" },
    { id: "p7", sku: "COIL-MT-06", name: "MistTech Mesh Coil 0.6Ω (5pk)", categoryId: "c4", brandId: "b3", cost: 8.0, price: 16.99, stock: 55, unit: "pack" },
    { id: "p8", sku: "POD-NIM-2P", name: "NimbusHouse Pod Cartridge (2pk)", categoryId: "c4", brandId: "b5", cost: 7.0, price: 13.99, stock: 2, unit: "pack" },
    { id: "p9", sku: "ACC-BAT-186", name: "18650 Battery 3000mAh", categoryId: "c5", brandId: "b6", cost: 4.5, price: 9.99, stock: 61, unit: "unit" },
    { id: "p10", sku: "ACC-CHG-DL", name: "Dual-Bay Battery Charger", categoryId: "c5", brandId: "b6", cost: 8.8, price: 17.99, stock: 9, unit: "unit" },
    { id: "p11", sku: "SLT-STRW-30", name: "Strawberry Salt 30ml 35mg", categoryId: "c6", brandId: "b1", cost: 7.0, price: 15.99, stock: 33, unit: "bottle" },
    { id: "p12", sku: "SLT-MINT-30", name: "Arctic Mint Salt 30ml 35mg", categoryId: "c6", brandId: "b1", cost: 7.0, price: 15.99, stock: 0, unit: "bottle" },
    { id: "p13", sku: "ELQ-PNCH-60", name: "Fruit Punch 60ml", categoryId: "c1", brandId: "b4", cost: 10.5, price: 22.99, stock: 19, unit: "bottle" },
    { id: "p14", sku: "MOD-VV-KIT2", name: "VoltVape Pro Kit 2.0", categoryId: "c3", brandId: "b2", cost: 27.0, price: 54.99, stock: 5, unit: "unit" },
  ],
  ...
  stockOut: [
    { id: "so1", date: "2026-08-05", productId: "p1", qty: 6, channel: "In-store", note: "" },
    { id: "so2", date: "2026-08-05", productId: "p9", qty: 12, channel: "In-store", note: "" },
    { id: "so3", date: "2026-08-06", productId: "p3", qty: 4, channel: "Online", note: "" },
    { id: "so4", date: "2026-08-06", productId: "p7", qty: 8, channel: "In-store", note: "" },
    { id: "so5", date: "2026-08-07", productId: "p11", qty: 5, channel: "Online", note: "" },
    { id: "so6", date: "2026-08-07", productId: "p4", qty: 3, channel: "In-store", note: "" },
  ],
};
...
const PAGE_META = {
  dashboard: { title: "Dashboard", sub: "Today's snapshot across the shop" },
  ...
  reports: { title: "Reports", sub: "Sales, stock movement, and margins" },
  settings: { title: "Settings", sub: "Shop preferences and thresholds" },
};
...
function navigateTo(pageId) {
  ...
  $("#primaryAction").style.display = ["dashboard", "low-stock", "reports", "settings"].includes(pageId) ? "none" : "inline-flex";
  $("#primaryAction").dataset.for = pageId;
  const labels = { products: "New product", categories: "New category", brands: "New brand", suppliers: "New supplier", "stock-in": "Receive stock", "stock-out": "Record sale" };
...
function renderDashboard() {
  ...
  const stockValue = state.products.reduce((sum, p) => sum + p.stock * p.price, 0);
  const lowStockCount = state.products.filter((p) => p.stock <= state.settings.lowStockThreshold).length;
  const todaysDate = "2026-08-07";
  const todaysOuts = state.stockOut.filter((o) => o.date === todaysDate);
  const todaysSales = todaysOuts.reduce((sum, o) => sum + o.qty * (productById(o.productId)?.price || 0), 0);

  $("#statGrid").innerHTML = [
    StatCard({ label: "Total Products", value: totalProducts, icon: ICONS.box, colorClass: "teal", trend: { dir: "up", icon: "▲", text: "6 categories active" } }),
    StatCard({ label: "Stock Value", value: money(stockValue), icon: ICONS.chart, colorClass: "violet", trend: { dir: "up", icon: "▲", text: "at current retail price" } }),
    StatCard({ label: "Low Stock Items", value: lowStockCount, icon: ICONS.alert, colorClass: "amber", trend: { dir: lowStockCount ? "down" : "up", icon: lowStockCount ? "▼" : "▲", text: lowStockCount ? "needs reordering" : "all healthy" } }),
    StatCard({ label: "Today's Sales", value: money(todaysSales), icon: ICONS.tag, colorClass: "teal", trend: { dir: "up", icon: "▲", text: `${todaysOuts.length} transactions` } }),
  ].join("");

  // last 7 days sales bar chart (mocked spread across stockOut dates + filler)
  const days = ["Aug 1", "Aug 2", "Aug 3", "Aug 4", "Aug 5", "Aug 6", "Aug 7"];
  const values = [312, 428, 260, 505, 611, 388, Math.round(todaysSales) || 190];
  ...
  $("#topProductsList").innerHTML = top.length
    ? top.map(([pid, qty]) => {
        ...
        return `
          <div class="mini-row">
            <div class="mini-thumb">${esc(p.name.slice(0, 2).toUpperCase())}</div>
            <div class="mini-main">
  ...
  // low stock preview
  const low = state.products.filter((p) => p.stock <= state.settings.lowStockThreshold).sort((a, b) => a.stock - b.stock).slice(0, 5);
  $("#lowStockPreview").innerHTML = low.length
    ? low.map((p) => `
        <div class="mini-row">
          <div class="mini-thumb">${esc(p.name.slice(0, 2).toUpperCase())}</div>
          <div class="mini-main">
  ...
  // recent stock movement (combined)
  const movement = [
    ...state.stockIn.map((m) => ({ ...m, type: "in" })),
    ...state.stockOut.map((m) => ({ ...m, type: "out" })),
  ...
  renderTable({
    ...
    cellFn: (row, key) => {
      ...
      if (key === "type") return row.type === "in" ? Badge("Stock In", "ok") : Badge("Stock Out", "info");
      if (key === "qty") return `<span class="cell-mono">${row.type === "in" ? "+" : "−"}${row.qty}</span>`;
      return "";
    },
  });
}
  ...
  renderTable({
    ...
    cellFn: (r, key) => {
      ...
      switch (key) {
        ...
        case "product": return `<span class="cell-strong">${esc(p?.name || "—")}</span><div class="cell-sub">${esc(p?.sku || "")}</div>`;
        case "qty": return `<span class="cell-mono">−${r.qty}</span>`;
        case "channel": return Badge(r.channel, r.channel === "Online" ? "info" : "neutral");
...
function renderReports() {
  const totalRevenue = state.stockOut.reduce((sum, o) => sum + o.qty * (productById(o.productId)?.price || 0), 0);
  const totalCOGS = state.stockOut.reduce((sum, o) => sum + o.qty * (productById(o.productId)?.cost || 0), 0);
  const grossProfit = totalRevenue - totalCOGS;
  const unitsSold = state.stockOut.reduce((sum, o) => sum + o.qty, 0);
  const unitsReceived = state.stockIn.reduce((sum, o) => sum + o.qty, 0);

  $("#reportStatGrid").innerHTML = [
    StatCard({ label: "Total Revenue", value: money(totalRevenue), icon: ICONS.chart, colorClass: "teal" }),
    StatCard({ label: "Gross Profit", value: money(grossProfit), icon: ICONS.award, colorClass: "violet" }),
    StatCard({ label: "Units Sold", value: unitsSold, icon: ICONS.arrowDown, colorClass: "amber" }),
    StatCard({ label: "Units Received", value: unitsReceived, icon: ICONS.arrowUp, colorClass: "teal" }),
  ].join("");

  // revenue by category
  const revByCat = {};
  state.stockOut.forEach((o) => {
  ...
  state.stockIn.forEach((si) => { spendBySupplier[si.supplierId] = (spendBySupplier[si.supplierId] || 0) + si.qty * si.unitCost; });
  ...
  // margin by product (top 6 by margin %)
...
function bindSettingsForm() {
  ...
  $("#settingsResetBtn").addEventListener("click", () => {
    confirmDelete("Reset settings to their defaults?", () => {
      state.settings.shopName = "Nimbus Vape Co.";
      state.settings.currency = "$";
      state.settings.taxRate = 8;
      state.settings.lowStockThreshold = 10;
      state.settings.criticalThreshold = 4;
      state.settings.notifyLowStock = true;
      state.settings.notifyDailySummary = true;
      state.settings.autoReorder = false;
      renderSettings();
      $("#brandNameLabel").textContent = state.settings.shopName;
